...
export function PlanCavernoso({
  ...
  console.log(showAllBenefits, isAnnualPlan, isOnboarding)
  return (
    <div
      className={cn(
        'flex flex-col transition-all duration-700 overflow-hidden w-full bg-zinc-900 rounded-3xl border-2 border-red-500',
        showAllBenefits || isAnnualPlan ? 'h-auto' : 'h-full',
        isOnboarding && '3xl:h-[650px] h-[550px]',
      )}
    >
      <div className="flex flex-col relative gap-2">
        {(isOnTrial || isInactive) && (
        ...
        >
          ...
          {isAnnualPlan && (
            <div className="grid grid-cols-2 gap-4">
              {PLAN_FEATURES.slice(0, 20).map((feature) => (
                <li key={feature} className="flex items-center gap-3">
                  <Check size={20} className="text-zinc-400" />
                  <span className="text-zinc-300 text-sm">{feature}</span>
                </li>
