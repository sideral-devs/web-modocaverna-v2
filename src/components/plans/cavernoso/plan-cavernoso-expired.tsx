...
export function PlanCavernosoYearlyExpired({
  selectedPlan,
  setSelectedPlan,
  isUpdatePlan,
  getPlanUrl,
  ...
  const initialFeaturesToShow = selectedPlan === 'monthly' ? 6 : 5
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col h-fit border-t border-x border-b bg-zinc-800 rounded-3xl">
        ...
        {!showAllBenefits && (
          <div
            className={`sticky bottom-0 flex items-end justify-center w-full rounded-full py-[26px] bg-gradient-to-t from-zinc-800 via-zinc-800/90 to-transparent`}
          >
            <button
              onClick={() => setShowAllBenefits(!showAllBenefits)}
              className="text-red-500 cursor-pointer font-medium text-sm"
            >
              Ver todos os benefícios anual
            </button>
          </div>
        )}
      </div>
